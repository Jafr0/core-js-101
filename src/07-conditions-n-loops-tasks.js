...
function isInsideCircle(circle, point) {
  const rad1 = point.x - circle.center.x;
  const rad2 = point.y - circle.center.y;
  if (rad1 ** 2 + rad2 ** 2 >= circle.radius ** 2) return false;
  return true;
}
...
function getDigitalRoot(num) {
  ...
  const brr = arr.reduce((a, b) => a / 1 + b / 1);
  return brr > 9 ? getDigitalRoot(brr) : brr;
}
...
function isBracketsBalanced(str) {
  ...
  const arr = [];
  const brr = str.split('');
  for (let i = 0; i < brr.length; i += 1) {
    if (fls[brr[i]]) arr.push(brr[i]);
    else if (nice[arr.pop()] !== brr[i]) {
      return false;
    }
  }
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < position.length; i += 1) {
    ...
    } else if (position[0][i] === position[1][i] && position[0][i] === position[2][i]) {
      if (position[0][i]) {
        return position[0][i];
      }
    ...
    } else if (position[2][0] === position[1][1] && position[2][0] === position[0][2]) {
      if (position[2][0]) {
        return position[2][0];
      }
    }
